...
IPADDR=$(ip -4 addr show "$INTERFACE" | grep -oP "(?<=inet ).*(?=/)")
...
function getMemStatus() {
    ...
    echo "内存总容量(MB):$report_MemTotal"
    echo "内存剩余(MB):$report_MemFree"
...
function getDiskStatus() {
    ...
    echo "硬盘总容量(GB):$report_DiskTotal"
    echo "硬盘剩余(GB):$report_DiskFree"
